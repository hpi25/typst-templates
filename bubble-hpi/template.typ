...
#let bubble(
  ...
  logo: image("assets/logo_full.svg"),
  main-color: "de6207",
  alpha: 60%,
  color-words: (),
  body,
  ...
  let secondary-color = color.mix(
    color.rgb(100%, 100%, 100%, alpha),
    primary-color,
    space: rgb,
  )

  // highlight important words
  show regex(if color-words.len() == 0 { "$ " } else {
    color-words.join("|")
  }): text.with(fill: primary-color)
  ...
  show raw.where(block: false): it => (
    h(0.5em)
      + box(fill: primary-color.lighten(90%), outset: 0.2em, it)
      + h(0.5em)
  )

  // Set body font family.
  set text(font: body-font, 12pt, weight: "regular")
  ...
  set enum(indent: 1em, numbering: n => [#text(fill: primary-color, numbering(
    "1.",
    n,
  ))])
  ...
  place(top + left, dx: -35%, dy: -28%, circle(
    radius: 150pt,
    fill: primary-color,
  ))
  ...
  place(bottom + right, dx: 40%, dy: 30%, circle(
    radius: 150pt,
  ...
  align(center, text(font: title-font, title-size, weight: 700, title))
  v(2em, weak: true)
  if subtitle != none {
    align(center, text(font: title-font, subtitle-size, weight: 700, subtitle))
    v(2em, weak: true)
  }
  ...
  // Main body.
  set page(
    header: text(10pt, [#header-content #h(1fr) #date]),
  )
